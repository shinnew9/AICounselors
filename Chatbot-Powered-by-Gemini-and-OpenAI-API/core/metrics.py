    ...
    for raw in md.splitlines():
        ...
        if m:
            ...
            if m.group(3) is not None:
                out[key]["score"] = int(m.group(3)); continue
        ...
        if m2:
            ...
            if m2.group(2) is not None: out["Advice Timing"]["score"] = int(m2)
    return out
...
JSON:"""
    ...
    try:
        ...
        data = {}
    norm = {k: 1 if str(data.get(k, 0)).strip().lower() in {"1", "true"} else 0
            for k in ["empathy", "reflection", "validation", "open_question", "suggesiton"]}
    return norm
...
def compute_session_skill_rates(labels: list[dict]) -> dict:
    ...
    for d in labels:
        ...
        sums["Suggestions"] += d.get("suggestion",0)

    return {k: round(v/n, 4) for k, v in sum.items()}